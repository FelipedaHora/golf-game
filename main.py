...
def endScreen(): # Exibir esta tela quando o usuário concluir o curso
    global start, starting, level, sheet, coins
    starting = True
    start = True

    # Desenhe todo o texto para exibir na tela
    win.blit(background, (0,0))
    text = myFont.render('Fase Completa!', 1, (64,64,64))
    ...
    text = parFont.render('Batidas: ' + str(sheet.getStrokes()), 1, (64,64,64))
    win.blit(text, ((winwidth/2 - text.get_width()/2, 280)))
    blit = parFont.render('Aperte o mouse para continuar...', 1, (64, 64, 64))
    win.blit(blit, (winwidth/2 - blit.get_width()/2, 510))
    text = parFont.render('Ponto: ' + str(sheet.getScore()), 1, (64,64,64))
    win.blit(text, ((winwidth/2 - text.get_width()/2, 360)))
    text = parFont.render('Moedas coletadas: ' + str(coins), 1, (64,64,64))
    win.blit(text, ((winwidth/2 - text.get_width()/2, 470)))
    pygame.display.update()


    # REESCREVER ARQUIVO DE TEXTO Contando Pontuações
    oldscore = 0
    oldcoins = 0
...
def setup(level):  # Configurar objetos para o nível dos cursos do módulo
...
def fade():  # Tela esmaecida quando o jogador coloca a bola no buraco
...
def showScore():  # Exibir a pontuação da classe planilha
    global level
    sleep(2)
    pygame.quit()
    level = 1
...
def holeInOne():  # Se o jogador acertar um buraco em uma mensagem especial na tela
...
def redrawWindow(ball, line, shoot=False, update=True):
    ...
    text = parFont.render('Par: ' + str(par), 1, (255,255,255))
    win.blit(text, (20,10))
    text = parFont.render('Batidas: ' + str(strokes), 1, (255,255,255))
    win.blit(text, (18,45))
...
def coinImg():  # Animação para moeda giratória, moeda atua como moeda
    global coinTime, coinIndex
    coinTime += 1
    if coinTime == 15:  # variável de contagem baseada na velocidade do clock
        coinIndex += 1
        coinTime = 0
    if coinIndex == 8:
        coinIndex = 0
...
# Encontre o ângulo em que a bola atinge o solo
def findAngle(pos):
    sX = ballStationary[0]
    sY = ballStationary[1]
    try:
...
def onGreen():  # Verifica se a bola está na zona verde
    global hole
...
def overHole(x,y):  # Verifica se a bola está em cima do buraco
...
# Game Loop para níveis e colisões
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # Trava o angulo e inicia a contagem da força
            if lock == 0:
                powerAngle = math.pi
                neg = 1
                powerLock = False
                ...
                while not powerLock:  # Caso não tenha pressionado o botão, continua o loop de força
                    loopTime += 1
                    if loopTime == 6:
                        powerAngle -= 0.1 * neg
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if put:
                                ...
                                if not superPower:  # Muda a potência se selecionar o power ball
        ...
        if event.type == pygame.MOUSEMOTION:  # Altera a posição da linha angular
            pos = pygame.mouse.get_pos()
            angle = findAngle(pos)
            line = (round(ballStationary[0] + (math.cos(angle) * 50)), round(ballStationary[1] - (math.sin(angle) * 50)))

            if onGreen():  # Se estivermos no verde e o ângulo fica travado em direção ao buraco
    ...
    while put and not shoot:  # If we are putting
        if not(overHole(ballStationary[0], ballStationary[1])):
            ...
            rollVel -= 0.5  # Desacelera a bola gradativamente
            if angle == math.pi:
            ...
            if rollVel < 0.5:  # Para a bola se a potência for baixa
                time = 0
                put = False
                pos = pygame.mouse.get_pos()
                angle = findAngle(pos)
                ...
                #Determina para que lado apontar a linha de direção
                if onGreen():
                    if ballStationary[0] > flagx:
                        angle = math.pi
                        line = (ballStationary[0] - 30, ballStationary[1])
            ...
            while True:  # Animação da bola no buraco
                pygame.time.delay(20)
                redrawWindow(ballStationary, None, True)
    ...
    while shoot:  # Enquanto estiver "atirando"
        if not(overHole(ballStationary[0], ballStationary[1])): 
            ...
            for i in objects:  # Para todos os objetos presentes no cenário
                if i[4] == 'coin':  
                    if i[5]:
                ...
                #         break
                    ...
                    if ballCords[1] > i[1] - 6 and ballCords[1] < i[1] + 8 and ballCords[0] < i[0] + i[2] and ballCords[0] > i[0] + 2:
                        ...
                        label = myFont.render('Acertou a água, +1 batida', 1, (255, 255, 255))
                        win.blit(label, (winwidth / 2 - label.get_width() / 2, winheight / 2 - label.get_height() / 2))
                        pygame.display.update()
